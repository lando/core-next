...
module.exports = async (event, data, hooks = [], context = {}, id = 'lando', errorHandler) => {
  // debugger
  const debug = require('debug')(`${id}:@lando/core:runHook`);
  ...
  const promises = hooks.map(async group => {
    ...
    const context = {...context, debug: require('debug')([id, 'runHook', group.name || group.id, 'hooks', event].join(':'))};
    ...
    for (const runner of runners) {
      try {
        ...
        const result = await module.call(context, {...data, ...context});
