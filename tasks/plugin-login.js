...
module.exports = (lando) => {
  return {
    ...
    examples: ['$0 plugin-login --username riker --password "$TOKEN"', '$0 plugin-login --registry https://npm.pkg.github.com'],
    level: 'tasks',
    options: {
      username: {
    ...
    run: async (options) => {
      ...
      try {
        ...
        lando.log('wrote plugin config to %s', lando.config.pluginConfigFile);
        ...
        if (error.statusCode) {
          ...
          lando.log('%s request failed to %s with status code %s', error.method, error.uri, error.statusCode);
          throw error;
