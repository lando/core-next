...
class AsyncEvents extends EventEmitter {
  ...
  on(name, priority, fn) {
    ...
    // Log
    this.log('loading event %s priority %s', name, priority);
  ...
  emit(...args) {
    ...
    this.log('emitting event %s', name);
    this.log('event %s has %s listeners', name, fns.length);

    // Make listener functions to a promise in series.
    return (
      Promise.each(fns, (listener) => {
