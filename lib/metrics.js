...
module.exports = class Metrics {
  constructor({ id = 'unknown', log = debug, endpoints = [], data = {} } = {}) {
    this.id = id;
    this.log = log;
  ...
  report(action = 'unknown', data = {}) {
    ...
    return (
      Promise.resolve(this.endpoints)
        // Filter out any inactive endpoints
        .filter((endpoint) => endpoint.report)
        // Get the client and report
        .map((endpoint) => {
          // Log the attempt
          log('reporting %s action to', action, this.endpoints);
          log('reporting data', send);
          ...
          return agent.post('/metrics/v2/' + id, send).catch((error) => {
            ...
            log('metrics post to %s failed with %s (%s) %s', url, status, reason, message);
          });
        })
    );
  }
