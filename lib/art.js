...
exports.appDestroy = ({ name, phase = 'pre' } = {}) => {
  switch (phase) {
    ...
    case 'post':
      return [chalk.red(`The app known as ${italicize(name)} has paid the ${chalk.bold('IRON PRICE')}. App destroyed!`)].join(os.EOL);
  }
};
...
exports.appStart = ({ name, phase = 'pre', messages = [] } = {}) => {
  switch (phase) {
    ...
    case 'post_legacy':
      return ['', getHeader(messages), '', 'Your app has started up correctly.', 'Here are some vitals:', ''].join(os.EOL);
    case 'report_legacy': {
      const reportLegacy = [
        '',
        getHeader(messages),
...
exports.experimental = (on = false) => {
  switch (on) {
    case true:
      return ['', chalk.green(niceFont('Activated!!!', 'Small Slant')), chalk.magenta('Experimental features are now ON'), ''].join(os.EOL);
    case false:
      return ['', chalk.red(niceFont('Deactivated!', 'Small Slant')), chalk.grey('Experimental features are now OFF'), ''].join(os.EOL);
  }
};
...
exports.poweroff = ({ phase = 'pre' } = {}) => {
  switch (phase) {
    case 'pre':
      return ['', chalk.cyan('NO!! SHUT IT ALL DOWN!!!'), chalk.magenta(niceFont('Powering off...', 'Small Slant')), ''].join(os.EOL);
    case 'post':
...
exports.secretToggle = (on = false) => {
  switch (on) {
    ...
    case false:
      return ['', chalk.red(niceFont('Deactivated!', 'Small Slant')), chalk.grey('The secret toggle is now OFF'), ''].join(os.EOL);
  }
};
 ...
 */
exports.sudoRun = () =>
  [chalk.red('Lando should never ever ever be run as root...'), chalk.magenta(niceFont('like ever!!!', 'Small Slant'))].join(os.EOL);
