...
const bootstrapConfig = async (lando) => {
  ...
  if (!plugins.find((plugin) => plugin.name === '@lando/core')) {
    ...
    lando.log('no core detected loading from %o', lando.config.coreBase);
  ...
  // log
  if (!_.isEmpty(removed)) lando.log('removed duplicate plugin entries %o', removed);
...
const bootstrapEngine = (lando) => {
  ...
  lando.engine = require('../utils/setup-engine')(lando.config, lando.cache, lando.events, lando.log, lando.shell, lando.config.instance);
...
const bootstrapApp = (lando) => {
  ...
  const legacyBuilders = _(['compose', 'types', 'services', 'recipes'])
    ...
    .value();
  _.forEach(legacyBuilders, (builder) => lando.log('autoloaded legacy builder %s', builder));
  ...
  const legacyItems = _(['builders'])
    ...
    .value();
  _.forEach(legacyItems, ({ name, api }) => lando.log('autodiscovered legacy api %s builder %s', api, name));
...
const bootstrapRouter = async (level, lando) => {
  switch (level) {
    case 'config':
      return await bootstrapConfig(lando);
    case 'tasks':
...
module.exports = class Lando {
  ...
  bootstrap(level = 'app') {
    ...
    this.log('starting bootstrap at level %s...', level);
    this.log("it's not particularly silly, is it?");
    ...
    return this.Promise.each(bootstraps, (level) => {
      this.log('%s bootstrap beginning...', level);
      ...
      return (
        this.events
          ...
          // Log the doneness
          .then(() => this.log('%s bootstrap completed.', level))
      );
    })
      .then(() => this.log('bootstrap completed.'))
  ...
  async generateCert(
    name,
    { caCert = this.config.caCert, caKey = this.config.caKey, domains = [], organization = 'Lando Alliance', validity = 365 } = {},
  ) {
    ...
    this.log('received cert request for %o with names %j using CA %o', name, domains, caCert);
    ...
    write(certPath, cert);
    write(keyPath, key);
    this.log('generated cert/key pair %o %o', certPath, keyPath);
  ...
  getApp(startFrom = process.cwd(), warn = true) {
    ...
    // @TODO; this needs to end up in the CLI
    // Return warning if we find nothing
    if (_.isEmpty(landoFiles)) {
      if (warn) {
        console.warn('could not find app in this dir or a reasonable amount of directories above it!');
      }
      return;
    }
    ...
    this.log('loading app %s from config files', config.name, landoFiles);
    // Return us some app!
  ...
  async installPlugins(options = this.config.setup) {
    ...
    this.log('installed %s of %s plugins with %s errors', results.length, total, errors.length);
  ...
  async runTasks(tasks, options = {}) {
    // some defaults
    const defaults = { rendererOptions: { log: this.log } };
    ...
    if (this.config.logLevelConsole > 3 || this.config.logLevelConsole === 'debug' || this.config.logLevelConsole === 'silly') {
  ...
  async setup(options = this.config.setup) {
    ...
    if (options.installTasks) {
      ...
      data.results = [...data.results, ...results];
      data.total = data.total + total;
      this.log('ran %s of %s tasks with %s errors', results.length, results.length, errors.length);
    }
    ...
    data.restart =
      ...
      options.tasks.filter((task) => task.enabled).some((task) => task.requiresRestart === true);
