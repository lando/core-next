...
const autoLoaders = ['app.js', 'builders', 'compose', 'inits', 'methods', 'scripts', 'services', 'sources', 'recipes', 'tasks', 'types'];
...
module.exports = class Plugins {
  constructor(log = debug('lando:plugins')) {
    this.registry = [];
    this.log = log;
  }
  ...
  load(plugin, file = plugin.path, ...injected) {
    ...
    try {
      ...
      this.log('problem loading plugin %o from %o: %o', plugin.name, file, e.stack);
    }

    // Register, log, return
    this.registry.push(plugin);
    this.log('plugin %o loaded from %o', plugin.name, file);
    this.log('plugin %o has', plugin.name, plugin.data);
