 ...
 */
const addCommand = ({ cmd, id, mode = 'exec', process = {} } = {}) => _.merge({}, parseCmd(parse(cmd)), { id, mode, process });
...
module.exports = class Shell {
  constructor(log = debug('lando:shell')) {
    this.log = log;
    this.running = [];
  ...
  sh(cmd, { mode = 'exec', detached = false, cwd = process.cwd(), cstdio = 'inherit', silent = false } = {}) {
    ...
    this.log('process %s running %s', id, strippedCmd, { cstdio, silent, mode, detached });
    this.log('process %s full command, running', id, parse(cmd));
    ...
    return (
      Promise.try(() => {
        ...
        .then(({ code, stdout, stderr }) => {
          ...
          // Log
          this.log('process %s finished with exit code %s', id, code);
          this.log('process %s had output', id, { stdout, stderr });
          // Return
