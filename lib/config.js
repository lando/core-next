...
class Config extends nconf.Provider {
  ...
  constructor({
    ...
    // log
    this.debug('initializing config %o with options %o', this.id, {cached, decode, encode, managed, env: this.env});

    // start with a memcache populated with data
    // @TODO: what about encoding?
    this.add('#mem', {type: 'memory', store: data, readOnly: false});
  }
  ...
  dump() {
    ...
    write(this.cached, this.getUncoded());
  ...
  get(path, options, data, store) {
    ...
    // @TODO: array merge goes here?

    // otherwise get the merged data
    } else data = merge({}, ...Object.keys(this.stores).reverse().map(store => this.stores[store].store));
  ...
  getRaw(path) {
    return this.get(path, {encode: false, decode: false});
  };

  // return uncoded
  getUncoded(path) {
    return this.get(path, {encode: true, decode: false});
  };

  // @NOTE: this is just a POC for config2 and so we can get get thigns working again
  // @TODO: what strats here? replace, last, first, combine, union
  // @TODO: what about removing dups? empties etc?
  getArrayByMerge(path, {strategy = 'default', by = 'id'} = {}) {
    // start by getting array of data
    const data = Object.entries(this.stores).map(store => store[1].get(path)).filter(Boolean).reverse();
    // reduce to a merged object
    const obj = data.reduce((acc, datum) => {
      return merge(acc, Object.fromEntries(datum.map(a => [a[by], a])));
    }, {});
    // transform back to array and return
    return Object.entries(obj).map(data => data[1]);
  };
  ...
  save(data, store = this.managed, options) {
    ...
    // otherwise to the usual saving
      ...
      this.stores[store].store = merge({}, this.stores[store].store, data);
      write(dest, this.stores[store].store);
    }
