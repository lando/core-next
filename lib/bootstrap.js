...
class Bootstrapper {
  ...
  #_reint() {
    this.#clearInternalCache(['hooks', 'plugins.disabled', 'plugins.enabled', 'plugins.invalid', 'registry']);
    this.#_init();
  }
  ...
  constructor(config = {}) {
    ...
    this.#_cache = new FileStorage(({debug: this.debug.extend('#cache'), dir: this.config.get('system.cache-dir')}));
  ...
  getHooks() {
    ...
    if (this.#_cache.get('hooks')) {
      ...
      this.debug('grabbed %o %o hooks(s) from hooks cache', hooks.length, this.id);
      return hooks;
    }
  ...
  getPlugins(options = {}) {
    // if we have something cached then just return that
    if (this.#_cache.get('plugins.enabled')) {
      const plugins = this.#_cache.get('plugins.enabled');
