...
class Bootstrapper {
  static debug = require('debug')('bootstrap');
  ...
  constructor(config = {}) {
    ...
    // debugger
    this.debug = require('../utils/get-debugger')(this.id, {extend: config.debug});

    // add some helpful things
    // some sort of library mix in to override things?
    Config.id = this.id;
    Plugin.id = this.id;
    ...
    this.#_cache = new FileStorage(({dir: this.config.get('system.cache-dir')}));
