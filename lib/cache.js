...
const debug = require('../_new/lib/debug.js').default;

/*
 * Creates a new Cache instance.
 */
class Cache extends NodeCache {
  constructor({ log = debug('lando:cache'), cacheDir = path.join(os.tmpdir(), '.cache') } = {}) {
  ...
  set(key, data, { persist = false, ttl = 0 } = {}) {
    ...
    if (this.__set(key, data, ttl)) {
      this.log('cached %j with key %o for %o', data, key, { persist, ttl });
    } else {
      this.log('failed to cache %j with key %o', data, key);
    }
  ...
  get(key) {
    ...
    if (memResult) {
      this.log('retrieved from memcache with key %o', key);
      return memResult;
    } else {
      try {
        this.log('trying to retrieve from file cache with key %o', key);
        return jsonfile.readFileSync(path.join(this.cacheDir, key));
      } catch (e) {
        this.log('file cache miss with key %o', key);
      }
    }
  }
  ...
  remove(key) {
    // Try to get cache
    if (this.__del(key)) this.log('removed key %o from memcache.', key);
    else this.log('failed to remove key %o from memcache.', key);

    // Also remove file if applicable
    try {
      fs.unlinkSync(path.join(this.cacheDir, key));
    } catch (e) {
      this.log('no file cache with key %o', key);
    }
  }
}
