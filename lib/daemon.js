...
const debug = require('../_new/lib/debug.js');
 ...
 */
const getMacProp = (prop) =>
  shell
    .sh(['defaults', 'read', `${MACOS_BASE}/Contents/Info.plist`, prop])
        ...
        try {
          switch (this.platform) {
            ...
            case 'linux': {
              ...
              if (password) await require('../utils/run-elevated')([lscript], { debug: this.debug, password });
              else await require('../utils/run-command')(lscript, { debug: this.debug });
              break;
            }
    ...
    return (
      this.events
        ...
        .then(() => {
          ...
          return (
            this.isUp(this.debug, this.cache, this.docker)
              .then((isUp) => {
    ...
    switch (this.platform) {
      ...
      case 'linux': {
        ...
        return shell
          .sh(cmd)
          .catch(() => '18.0.0')
          .then((version) => ({ ...versions, engine: version }));
      }
      case 'win32':
      case 'wsl': {
        const componentsVersionFile =
          this.platform === 'win32'
            ? path.resolve(getDockerBinPath('win32'), '..', 'componentsVersion.json')
            : '/Docker/host/componentsVersion.json';
