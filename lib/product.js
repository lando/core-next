...
class Product {
  ...
  constructor(config, {debug = Product.debug} = {}) {
    ...
    this._componentAlises = Object.fromEntries(Object.entries(this.config.getUncoded('core'))
      .filter(([type, component]) => typeof component === 'string')
  ...
  getComponent(component) {
    ...
    return require('../utils/get-component')(
      component,
      new Config({data: this.getRegistry(), id: `${this.id}-component-registry`}),
      {
  ...
  async getComponentInstance(component, constructor = {}, init = true) {
    return require('../utils/get-component-instance')(
      ...
      constructor,
      {
        ...
        registry: new Config({data: this.getRegistry(), id: `${this.id}-component-registry`}),
      },
    );
  }
  ...
  getHooks() {
    const hooks = this.Config.merge(this.hooks,
      [
        ...
        this.manifest.getUncoded(`hooks.${this.id}`, {ams: 'aoa'}),
      ],
      ['aoa'],
    );

    // debug
    this.debug('found %o and %o hooks %o', this.id, 'product', gos(this.hooks));
    // return
    return hooks;
  }
