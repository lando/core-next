...
const {normalizeArgv} = require('@oclif/core/help');
...
module.exports = class Cli {
  ...
  constructor({
    ...
    this.StorageBackend = this.cache ? require('../components/file-storage') : require('../components/no-storage');
    ...
    this.debug('instantiated cli with cache %o and hooks %o', this.cache, require('../utils/get-object-sizes')(this.hooks));
  }
  ...
  async run(argv = process.argv.slice(2), options = {}) {
    ...
    const ohooks = oclif.getPluginsList()
      .map(plugin => require('../utils/normalize-manifest-paths')({hooks: plugin.hooks}, plugin.root))
    ...
    this.debug('found early run oclif hooks %o', require('../utils/get-object-sizes')(this.hooks));
  ...
  async runHook(event, data) {
    return require('../utils/run-hook')(event, data, this.hooks, {cli: this}, this.debug, this.exitError);
  }
