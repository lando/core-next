...
class Plugin {
  ...
  static async fetch(plugin, {
    ...
    const info = await Plugin.info(pkg.raw);
    ...
    const {resolved} = await extract(pkg.raw, tmp, {Arborist: require('@npmcli/arborist')});
    Plugin.debug('extracted plugin %o to %o from %o', info._id, tmp, resolved);
    ...
    if (fs.existsSync(pjson)) write(pjson, merge(info, require(pjson)));
  ...
  constructor(location, {
    ...
    // add auxilary scope, project and spec
    this.project = this.name.split('@').length === 2 ? this.name.split('@')[1] : this.name.split('@')[0];
    this.scope = this.name.split('@').length === 2 ? this.name.split('@')[0] : undefined;
  ...
  async info() {
   ...
   // try to get info about the package
    ...
    Plugin.debug('retrieved plugin information for %o from %o', this.spec, info._resolved);
    return info;
  }
