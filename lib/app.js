...
module.exports = class App {
  constructor(name, config, lando = {}) {
     ...
     * @alias app.shell
     */
    this.log = debug(`lando:${this.project}`);
    this.shell = new Shell(this.log);
    this.engine = require('../utils/setup-engine')(lando.config, lando.cache, lando.events, this.log, this.shell, lando.config.instance);
  ...
  addWarning(message, error = null, hide = false) {
    ...
    if (error) {
      if (!hide) this.log(error.message || error);
      this.log(error);
  ...
  init({ noEngine = false } = {}) {
    ...
    // Log some things
    this.log('initiatilizing app at %s...', this.root);
    ...
    return (
      loadPlugins(this, this._lando)
        ...
        .then(() => {
          ...
          if (this.serviceGroups.compose.length === 0 && this.serviceGroups['3'].length === 0 && this.serviceGroups['4'].length > 0) {
            this._serviceApi = 4;
          }
  ...
  rebuild() {
    // Start it off
    this.log('rebuilding app...');
    return (
      initAndReport(this, 'rebuild')
        /**
  ...
  async runTasks(tasks, options = {}) {
    ...
    if (this._config.logLevelConsole > 3 || this._config.logLevelConsole === 'debug' || this._config.logLevelConsole === 'silly') {
