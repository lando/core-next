 ...
 */
const findProxyPorts = (lando, status) =>
  lando.Promise.try(() => {
...
const getFirstOpenPort = (scanner, urls = []) =>
 ...
 */
const normalizeRoutes = (services = {}) =>
  Object.fromEntries(
    _.map(services, (routes, key) => {
      ...
      routes = _(_.groupBy(routes, 'id'))
        .map((routes) => merge({}, routes, ['merge:key', 'replace']))
        .value();
 ...
 */
const parseConfig = (config, sslReady = []) =>
  _(config)
    .map((urls, service) => ({
      environment: {
...
module.exports = async (app, lando) => {
  ...
  if (lando.config.proxy === 'ON' && (!_.isEmpty(app.config.proxy) || !_.isEmpty(app.config.recipe))) {
    // generate proxy cert
    await lando.generateCert('proxy._lando_', { domains: [`*.${lando.config.proxyDomain}`, 'proxy._lando_.internal'] });
    ...
    // And then discover!
    return (
      findProxyPorts(lando, protocolStatus)
        .then((ports) => {
        ...
        .then(() => {
          ...
          if (_.max(_.values(urlCounts)) > 1) {
            app.log(
              'You cannot assign url %s to more than one service!',
              _.findKey(urlCounts, (c) => c > 1),
            );
          }
        ...
        .map((service) => {
          ...
          return {
            services: _.set({}, service.name, {
              ...
              volumes: [`${proxyVolume}:/proxy_config`, `${lando.config.userConfRoot}/scripts/proxy-certs.sh:/scripts/100-proxy-certs`],
            }),
        ...
        .then((result) => {
          ...
          app.log('app now has proxy compose files', app.compose);
        })
