...
module.exports = async (app) => {
  ...
  const estorage = _(await Promise.all(services.map(async (service) => await service.getStorageVolumes())))
    ...
    .value();
  app.log('found existing non-service scoped storage volumes %o', estorage);
  ...
  const cstorage = _(services)
    ...
    .value();
  app.log(
    'missing storage volumes %o',
    cstorage.map((volume) => volume.source),
  );

  // create any missing volumes
  await Promise.all(
    cstorage.map(async (volume) => {
      ...
      app.log('created %o storage volume %o with metadata %o', volume.scope, volume.source, volume.labels);
    }),
  );

  // run all build services methods
  app.log(
    'going to build v4 services',
  ...
  await Promise.all(
    services.map(async (service) => {
      try {
        ...
        // @TODO: improve this?
        app.log('app build error %o %o', error.message, error);
