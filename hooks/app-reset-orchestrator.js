...
module.exports = async (app, lando) => {
  ...
  if (lando._bootstrapLevel >= 3 && !app.engine.composeInstalled) {
    app.engine = require('../utils/setup-engine')(lando.config, lando.cache, lando.events, app.log, app.shell, lando.config.instance);
  }

  // log our sitch
  app.log('using docker-compose %s', lando.config.orchestratorBin);
};
