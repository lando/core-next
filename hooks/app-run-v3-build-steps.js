...
// Build keys
const preRootSteps = ['build_as_root_internal', 'build_as_root', 'install_dependencies_as_root_internal', 'install_dependencies_as_root'];
const preBuildSteps = ['build_internal', 'build', 'install_dependencies_as_me_internal', 'install_dependencies_as_me'];

// Post app start build steps
const postRootSteps = ['run_as_root_internal', 'run_as_root', 'extras'];
const postBuildSteps = ['run_internal', 'run_as_me_internal', 'run', 'run_as_me'];
...
module.exports = async (app, lando) => {
  ...
  const buildV3Services = _(app.parsedV3Services)
    ...
    .value();
  app.log('going to build v3 services if applicable', buildV3Services);
