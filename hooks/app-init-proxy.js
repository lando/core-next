...
module.exports = async (app, lando) => {
  if (lando.config.proxy === 'ON' && (!_.isEmpty(app.config.proxy) || !_.isEmpty(app.config.recipe))) {
    app.log('proxy settings detected.');

    lando.log('proxy is ON.');
    lando.log('Setting the default proxy certificate %s', lando.config.proxyDefaultCert);
    ...
    if (!_.isEmpty(lando.config.proxyCustom)) {
      lando.log('adding custom proxy config');
      lando.log('custom proxy config', lando.config.proxyCustom);
      files.push({
