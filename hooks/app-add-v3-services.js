...
module.exports = async (app, lando) => {
  ...
  app.parsedV3Services = _(app.parsedServices)
    .filter((service) => service.api === 3)
    .value();
  ...
  _.forEach(app.parsedV3Services, (service) => {
    ...
    if (_.isEmpty(_.find(lando.factory.get(), { api: 3, name: service.type }))) {
      app.log('%s is not a supported service type.', service.type);
    }
    // Log da things
    app.log('building v3 %s service %s', service.type, service.name);
    // Build da things
