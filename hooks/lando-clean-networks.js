...
module.exports = async (lando) =>
  lando.engine.getNetworks().then((networks) => {
    if (_.size(networks) >= lando.config.networkLimit) {
      // Warn user about this action
      lando.log("Lando has detected you are at Docker's network limit!");
      lando.log('Give us a moment as we try to make space by cleaning up old networks...');
      ...
      return (
        lando.engine
          ...
          // and remove it
          .each((net) => {
            lando.log('Removing old network %s', net.id);
            net.remove();
          })
      );
    }
