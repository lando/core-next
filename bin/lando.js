...
// @NOTE: you can set a custom value with --debug="*"
...
import createDebug from '../components/debug.js';
...
import getTasks from '../utils/get-tasks.js';
...
import Lando from '../lib/lando.js';
...
// debugger
const debug = createDebug(id || 'lando');
...
// start the preflight
debug('starting %o runtime version %o', id, pjson.version);
...
for (const file of config.configSources) {
  config = legacyMerge(config, loadFile(file));
...
if (config.envPrefix) {
  ...
  config = legacyMerge(config, data);
...
const cores = [path.join(config.userConfRoot, 'plugins', '@lando', 'core'), path.resolve(__dirname, '..')];

// if appConfig points to a different core lets set that here
if (typeof get(appConfig, 'plugins.@lando/core') === 'string') {
  cores.unshift(path.resolve(appConfig.root, appConfig.plugins['@lando/core']));
}

// return the first core that exists that we find
const COREBASE = find(cores, (core) => fs.existsSync(path.join(core, 'index.js')));

const main = async () => {
  if (!COREBASE) throw new Error('Unable to locate @lando/core runtime');
  ...
  // Lando cache stuffs
  process.lando = 'node';
  process.landoPlatform = isWslInterop() ? 'wsl' : process.platform;
  ...
  process.landoAppCacheFile = !isEmpty(appConfig) ? appConfig.composeCache : undefined;
  ...
  // determine bs level
  const bsLevel = !isEmpty(appConfig) && !fs.existsSync(process.landoAppCacheFile) ? 'APP' : 'TASKS';
  ...
  const bootstrappedLando = await lando.bootstrap(bsLevel);

  // If bootstrap level is APP then we need to get and init our app to generate the app task cache
  if (bsLevel === 'APP') {
    await bootstrappedLando.getApp().init({ noEngine: true });
  }
